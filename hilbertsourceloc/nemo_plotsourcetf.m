...
nmt_sourceplot_spm8(cfg,source_tf);
...
% if(isfield(source_tf,'dim'))
%     % FIXME: source_avg contains "dim", but others don't. this confuses
%     % sourceplot... long-term solution needed, but this is a temporary fix:
%     source = rmfield(source,'dim');
% end
% 
cfg.atlas = ft_read_atlas('ROI_MNI_V4.nii'); % AAL atlas
...
switch(masktype)
    ...
    case 'pval'
        pval = reshape([source_tf.pval{:}],length(source_tf.time),length(inside_idx))';
        source_tf.msk(inside_idx,1,:) = pval < maskthresh;
end
